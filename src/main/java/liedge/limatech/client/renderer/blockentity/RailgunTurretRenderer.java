...
import liedge.limatech.blockentity.RailgunTurretBlockEntity;
...
public class RailgunTurretRenderer extends TurretRenderer<RailgunTurretBlockEntity>
{
    ...
    {
        return LimaTechBlocks.RAILGUN_TURRET;
    }
    ...
    protected void renderAdditionalGuns(RailgunTurretBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight)
    {
        if (blockEntity.isLookingAtTarget())
        {
            ...
            if (dist > 0)
            {
                ...
                LimaColor color = blockEntity.lerpTicker(partialTick, 37) >= 1 ? LimaTechConstants.LIME_GREEN : LimaTechConstants.HOSTILE_ORANGE;

                LimaTechRenderUtil.renderPositionColorCuboid(buffer, mx4, lx - LASER_RADIUS, y - LASER_RADIUS, z, lx + LASER_RADIUS, y + LASER_RADIUS, z - dist, color, 0.8f, LimaTechRenderUtil.ALL_SIDES);
                LimaTechRenderUtil.renderPositionColorCuboid(buffer, mx4, rx - LASER_RADIUS, y - LASER_RADIUS, z, rx + LASER_RADIUS, y + LASER_RADIUS, z - dist, color, 0.8f, LimaTechRenderUtil.ALL_SIDES);
            }
        }
    }
}
...
public class LTXIJeiPlugin implements IModPlugin
{
    ...
    {
        registration.addRecipes(GRINDING_JEI, List.copyOf(LTXIClientRecipes.byType(GRINDING)));
        registration.addRecipes(MATERIAL_FUSING_JEI, List.copyOf(LTXIClientRecipes.byType(MATERIAL_FUSING)));
        registration.addRecipes(ELECTRO_CENTRIFUGING_JEI, List.copyOf(LTXIClientRecipes.byType(ELECTRO_CENTRIFUGING)));
        registration.addRecipes(MIXING_JEI, List.copyOf(LTXIClientRecipes.byType(MIXING)));
        registration.addRecipes(ENERGIZING_JEI, List.copyOf(LTXIClientRecipes.byType(ENERGIZING)));
        registration.addRecipes(CHEMICAL_REACTING_JEI, List.copyOf(LTXIClientRecipes.byType(CHEMICAL_REACTING)));
        registration.addRecipes(ASSEMBLING_JEI, List.copyOf(LTXIClientRecipes.byType(ASSEMBLING)));
        registration.addRecipes(GEO_SYNTHESIS_JEI, List.copyOf(LTXIClientRecipes.byType(GEO_SYNTHESIS)));
        registration.addRecipes(GARDEN_SIMULATING_JEI, List.copyOf(LTXIClientRecipes.byType(GARDEN_SIMULATING)));

        registration.addRecipes(FABRICATING_JEI, LTXIClientRecipes.getSortedFabricatingRecipes());
    }
    ...
    public void registerGuiHandlers(IGuiHandlerRegistration registration)
    {
        registration.addGuiContainerHandler(LimaMenuScreen.class, new IGuiContainerHandler<>()
        {
            ...
            public Optional<? extends IClickableIngredient<?>> getClickableIngredientUnderMouse(IClickableIngredientFactory builder, LimaMenuScreen containerScreen, double mouseX, double mouseY)
            {
                ...
                return builder.createBuilder(NeoForgeTypes.FLUID_STACK, slot.getFluid()).buildWithArea(containerScreen.getGuiLeft() + slot.x(), containerScreen.getGuiTop() + slot.y(), 16, 16);
            }
        });

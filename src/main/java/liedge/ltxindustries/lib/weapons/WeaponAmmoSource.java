...
public enum WeaponAmmoSource implements StringRepresentable, EffectTooltipProvider
{
    ...
    public void appendEffectLines(int upgradeRank, Consumer<Component> linesConsumer)
    {
        switch (this)
        {
            case COMMON_ENERGY_UNIT -> linesConsumer.accept(LTXILangKeys.ENERGY_AMMO_EFFECT.translate().withStyle(LTXIConstants.REM_BLUE.chatStyle()));
            case INFINITE -> LTXILangKeys.INFINITE_AMMO_EFFECT.translate().withStyle(LTXIConstants.CREATIVE_PINK.chatStyle());
        }
    }
}
...
public record ValueArgument(UpgradeDoubleValue value, ValueFormat format, ValueSentiment sentiment, int zero) implements TooltipArgument
{
    static final Codec<ValueArgument> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            UpgradeDoubleValue.CODEC.fieldOf("value").forGetter(ValueArgument::value),
    ...
    public static ValueArgument of(UpgradeDoubleValue value, ValueFormat format, ValueSentiment sentiment, int zero)
    {
        return new ValueArgument(value, format, sentiment, zero);
    }

    public static ValueArgument of(UpgradeDoubleValue value, ValueFormat format, ValueSentiment sentiment)
    {

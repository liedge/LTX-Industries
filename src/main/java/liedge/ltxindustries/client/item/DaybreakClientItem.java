package liedge.ltxindustries.client.item;

import com.mojang.blaze3d.pipeline.RenderPipeline;
import liedge.ltxindustries.item.weapon.WeaponItem;
import liedge.ltxindustries.lib.weapons.ClientExtendedInput;
import net.minecraft.client.gui.GuiGraphicsExtractor;
import net.minecraft.client.player.LocalPlayer;

public final class DaybreakClientItem extends WeaponClientItem
{
    public DaybreakClientItem()
    {
        super(5, 5, 10, 10);
    }

    @Override
    protected void extractCrosshairSprites(GuiGraphicsExtractor graphics, RenderPipeline pipeline, LocalPlayer player, WeaponItem weaponItem, ClientExtendedInput controls, int x, int y, float partialTick)
    {
        float bloom = 4f * applyCrosshairEasing(controls, weaponItem, partialTick);

        blitSprite(graphics, pipeline, HOLLOW_DOT, x, y, 5, 5, -1);

        blitSpriteMirrorV(graphics, pipeline, AOE_VERTICAL, x - 1, y - 4 - bloom, 7, 2, -1);
        blitSprite(graphics, pipeline, AOE_VERTICAL, x - 1, y + 7 + bloom, 7, 2, -1);

        blitSprite(graphics, pipeline, AOE_HORIZONTAL, x - 4 - bloom, y - 1, 2, 7, -1);
        blitSpriteMirrorU(graphics, pipeline, AOE_HORIZONTAL, x + 7 + bloom, y - 1, 2, 7, -1);
    }
}
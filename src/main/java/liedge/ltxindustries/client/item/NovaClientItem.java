...
public final class NovaClientItem extends WeaponClientItem
{
    ...
    protected void extractCrosshairSprites(GuiGraphicsExtractor graphics, RenderPipeline pipeline, LocalPlayer player, WeaponItem weaponItem, ClientExtendedInput controls, int x, int y, float partialTick)
    {
        ...
        blitSprite(graphics, pipeline, HEAVY_PISTOL_CROSSHAIR, xl, yu, 5, 5, -1);
        blitSpriteMirrorU(graphics, pipeline, HEAVY_PISTOL_CROSSHAIR, xr, yu, 5, 5, -1);
        blitSpriteMirrorU(graphics, pipeline, HEAVY_PISTOL_CROSSHAIR, xl, yd, 5, 5, -1);
        blitSprite(graphics, pipeline, HEAVY_PISTOL_CROSSHAIR, xr, yd, 5, 5, -1);

        if (aiming && triggerLerp == 0) graphics.fill(x, y, x + 1, y + 1, LTXIConstants.LIME_GREEN);
    }
}
...
public class DaybreakItem extends SemiAutoWeaponItem
{
    ...
    public void weaponFired(ItemStack heldItem, Player player, Level level, LTXIExtendedInput controls)
    {
        if (!level.isClientSide())
        {
            ...
            if (focusedTarget != null && controls.getTargetTicks() > 20) rocket.setTargetEntity(focusedTarget);

            level.addFreshEntity(rocket);
            level.gameEvent(player, LTXIGameEvents.WEAPON_FIRED, player.getEyePosition());
        }

        level.playSound(player, player, LTXISounds.DAYBREAK_FIRE.get(), SoundSource.PLAYERS, 2f, Mth.randomBetween(level.getRandom(), 0.75f, 0.9f));
    }
